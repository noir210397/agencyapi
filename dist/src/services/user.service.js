...
export async function getSingleUser(userId) {
    ...
    if (!user)
        throw new CustomError(StatusCode.Status404NotFound, null, "user was not found");
...
export async function deleteUser(userId, user) {
    ...
    if (deletedCount !== 1)
        throw new CustomError(StatusCode.Status404NotFound, null, "unable to delete user");
}
export async function updateUser(userId, user, body) {
    const { role, sub: id } = user;
    if (role === "AGENT") {
        ...
        if (!user)
            throw new CustomError(StatusCode.Status404NotFound, null, "user not found");
        if (user.role === "AGENT") {
            const { success, data, error } = updateAgentSchema.safeParse(body);
            if (!success)
                throw new CustomError(StatusCode.Status400BadRequest, error);
